...
function buildFileSuiteForProject(project, suite, repeatEachIndex) {
  ...
  result.forEachTest((test, suite) => {
    ...
    test.retries = (_inheritedRetries = inheritedRetries) !== null && _inheritedRetries !== void 0 ? _inheritedRetries : project.retries;
    test.timeout = (_inheritedTimeout = inheritedTimeout) !== null && _inheritedTimeout !== void 0 ? _inheritedTimeout : project.timeout;
    ...
    if (test._poolDigest) test._workerHash = `${project._internal.id}-${test._poolDigest}-${repeatEachIndex}`;
  });
  return result;
}
