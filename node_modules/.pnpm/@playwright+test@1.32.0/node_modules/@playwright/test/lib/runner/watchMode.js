...
class FSWatcher {
  ...
  async update(config) {
    const commandLineFileMatcher = config._internal.cliArgs.length ? (0, _util.createFileMatcherFromArguments)(config._internal.cliArgs) : () => true;
    ...
    for (const project of projectClosure) {
      const testMatch = (0, _util.createFileMatcher)(project.testMatch);
      const testIgnore = (0, _util.createFileMatcher)(project.testIgnore);
      projectFilters.set(project, file => {
        if (!file.startsWith(project.testDir) || !testMatch(file) || testIgnore(file)) return false;
        return project._internal.type === 'dependency' || commandLineFileMatcher(file);
      });
    }
    if (this._timer) clearTimeout(this._timer);
    if (this._watcher) await this._watcher.close();
    this._watcher = _utilsBundle.chokidar.watch(projectClosure.map(p => p.testDir), {
      ignoreInitial: true
...
async function runWatchModeLoop(config) {
  ...
  const reporter = await (0, _reporters.createReporter)(config, 'watch');
  const context = {
    config,
    reporter,
    phases: []
  ...
  if (status !== 'passed') return await globalCleanup();
  ...
  while (true) {
    ...
    if (command === 'file') {
      ...
      if (filePattern.trim()) config._internal.cliArgs = filePattern.split(' ');else config._internal.cliArgs = [];
      await fsWatcher.update(config);
      await runTests(config, failedTestIdCollector);
      lastRun = {
        type: 'regular'
    ...
    if (command === 'grep') {
      ...
      if (testPattern.trim()) config._internal.cliGrep = testPattern;else config._internal.cliGrep = undefined;
      await fsWatcher.update(config);
      await runTests(config, failedTestIdCollector);
      lastRun = {
        type: 'regular'
...
async function runTests(config, failedTestIdCollector, options) {
  ...
  const taskRunner = (0, _tasks.createTaskRunnerForWatch)(config, reporter, options === null || options === void 0 ? void 0 : options.projectsToIgnore, options === null || options === void 0 ? void 0 : options.additionalFileMatcher);
  const context = {
    config,
    reporter,
    phases: []
  ...
  for (const test of ((_context$rootSuite = context.rootSuite) === null || _context$rootSuite === void 0 ? void 0 : _context$rootSuite.allTests()) || []) {
  ...
  if (context.phases.find(p => p.dispatcher.hasWorkerErrors()) || hasFailedTests) status = 'failed';
...
function printConfiguration(config, title) {
  ...
  tokens.push(...((_ref = config._internal.cliProjectFilter || []) === null || _ref === void 0 ? void 0 : _ref.map(p => _utilsBundle2.colors.blue(`--project ${p}`))));
  if (config._internal.cliGrep) tokens.push(_utilsBundle2.colors.red(`--grep ${config._internal.cliGrep}`));
  if (config._internal.cliArgs) tokens.push(...config._internal.cliArgs.map(a => _utilsBundle2.colors.bold(a)));
