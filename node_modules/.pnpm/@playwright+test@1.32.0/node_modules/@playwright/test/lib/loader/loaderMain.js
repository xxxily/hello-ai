...
class LoaderMain extends _process.ProcessRunner {
  constructor(serializedConfig) {
    ...
    (0, _compilationCache.addToCompilationCache)(serializedConfig.compilationCache);
    this._serializedConfig = serializedConfig;
  }
  _config() {
    if (!this._configPromise) this._configPromise = _configLoader.ConfigLoader.deserialize(this._serializedConfig).then(configLoader => configLoader.fullConfig());
    return this._configPromise;
  }
  async loadTestFile(params) {
    const testErrors = [];
    const config = await this._config();
    const fileSuite = await (0, _testLoader.loadTestFile)(params.file, config.rootDir, testErrors);
