# 💼 商业与量化

> AI applied to financial analysis, quantitative trading, and enterprise.

> 当前分类已收录 **248** 个相关项目。

## Enterprise Solutions

### [FinGPT](https://github.com/AI4Finance-Foundation/FinGPT)

开源金融领域大语言模型，专注于金融数据分析、情感分析和智能投顾应用。

- **Stars:** ⭐️ 18.9k
- **Tags:** `Finance` `LLM` `NLP` `Fintech`
- **最后活动时间:** 2026-03-21

### [WrenAI](https://github.com/Canner/WrenAI)

生成式商业智能工具，支持自然语言查询数据库、自动生成 SQL 和图表。

- **Stars:** ⭐️ 14.7k
- **Tags:** `Text-to-SQL` `BI` `RAG`
- **最后活动时间:** 2026-03-26

### [FinRobot](https://github.com/AI4Finance-Foundation/FinRobot)

开源AI代理平台，专注于使用LLM进行金融分析。支持多模态深度学习和智能投顾应用。

- **Stars:** ⭐️ 6.5k
- **Tags:** `AI Agent` `Finance` `LLM` `Fintech`
- **最后活动时间:** 2026-03-23

### [supersonic](https://github.com/tencentmusic/supersonic)

新一代AI+BI平台，融合Chat BI（LLM驱动）与Headless BI（语义层驱动）范式，提供智能数据分析能力。

- **Stars:** ⭐️ 4.8k
- **Tags:** `BI平台` `数据分析` `LLM应用` `企业级`
- **最后活动时间:** 2026-03-25

### [EconML](https://github.com/py-why/EconML)

微软研究院开源的因果推断机器学习工具包，结合计量经济学与ML技术解决复杂因果问题。

- **Stars:** ⭐️ 4.6k
- **Tags:** `causal-inference` `econometrics` `machine-learning` `treatment-effects`
- **最后活动时间:** 2026-03-23

### [Financial-Knowledge-Graphs](https://github.com/jm199504/Financial-Knowledge-Graphs)

小型金融知识图谱构建流程教程，基于Neo4j与Python实现。

- **Stars:** ⭐️ 3.1k
- **Tags:** `knowledge-graph` `finance` `neo4j` `python`
- **最后活动时间:** 2024-07-07

### [AP2](https://github.com/google-agentic-commerce/AP2)

构建安全可互操作的AI驱动支付未来，支持代理式AI和账户对账户支付。

- **Stars:** ⭐️ 2.9k
- **Tags:** `Payments` `Agentic AI` `GenAI`
- **最后活动时间:** 2026-03-24

### [ML_Finance_Codes](https://github.com/mfrdixon/ML_Finance_Codes)

《Machine Learning in Finance》书籍配套代码，涵盖金融领域机器学习从理论到实践的完整内容。

- **Stars:** ⭐️ 2.5k
- **Tags:** `machine-learning` `finance` `trading`
- **最后活动时间:** 2020-06-13

### [ballerine](https://github.com/ballerine-io/ballerine)

开源风险决策平台，集成OCR身份验证和KYC/KYB合规流程。

- **Stars:** ⭐️ 2.4k
- **Tags:** `kyc` `identity-verification` `ocr` `risk-management` `orchestration`
- **最后活动时间:** 2026-03-26

### [FinGLM](https://github.com/MetaGLM/FinGLM)

开放的金融大模型项目，致力于推动AI与金融领域的深度融合与应用。

- **Stars:** ⭐️ 2.2k
- **Tags:** `Finance` `LLM` `ChatGLM` `GPT`
- **最后活动时间:** 2024-05-08

### [tensorflow-DeepFM](https://github.com/ChenglongChen/tensorflow-DeepFM)

基于TensorFlow的DeepFM模型实现，用于点击率预测的深度学习推荐算法。

- **Stars:** ⭐️ 2.1k
- **Tags:** `ctr-prediction` `deepfm` `recommendation`
- **最后活动时间:** 2018-06-10

### [CordysCRM](https://github.com/1Panel-dev/CordysCRM)

新一代开源AI CRM系统，深度融合数字化与智能化能力，支持私有化部署，配备AI助理提供7×24小时智能服务。

- **Stars:** ⭐️ 2.0k
- **Tags:** `ai-crm` `crm` `open-source` `enterprise`
- **最后活动时间:** 2026-03-24

### [steedos-platform](https://github.com/steedos/steedos-platform)

AI原生企业应用基础设施平台，支持通过提示词快速生成企业级软件，实现低代码/无代码开发。

- **Stars:** ⭐️ 1.5k
- **Tags:** `低代码` `企业应用` `AI原生`
- **最后活动时间:** 2026-03-26

### [Robyn](https://github.com/facebookexperimental/Robyn)

Meta开源的AI/ML驱动营销组合建模(MMM)工具包，帮助营销人员减少建模偏见、优化预算分配并 democratize 建模知识。

- **Stars:** ⭐️ 1.4k
- **Tags:** `marketing-mix-modeling` `machine-learning` `optimization` `econometrics`
- **最后活动时间:** 2026-01-26

### [FuxiCTR](https://github.com/reczoo/FuxiCTR)

可配置、可调优的点击率预测库，支持CTR/CVR预测任务，适用于推荐系统场景。

- **Stars:** ⭐️ 1.4k
- **Tags:** `ctr-prediction` `recommender-systems` `deep-learning` `pytorch`
- **最后活动时间:** 2025-06-16

### [BeeCount](https://github.com/TNT-Likely/BeeCount)

开源跨平台记账应用，支持iCloud/自建服务器同步，内置AI智能记账功能，隐私优先设计。

- **Stars:** ⭐️ 1.3k
- **Tags:** `Finance` `AI Bookkeeping` `Cross-platform` `Privacy-first`
- **最后活动时间:** 2026-03-24

### [jxwaf](https://github.com/jx-sec/jxwaf)

基于AI大模型的Web应用防火墙，结合传统WAF与AI能力提供智能安全防护。

- **Stars:** ⭐️ 1.2k
- **Tags:** `waf` `security` `nginx` `openresty` `ai-security`
- **最后活动时间:** 2026-02-25

### [zingg](https://github.com/zinggAI/zingg)

基于机器学习的可扩展实体解析与数据去重平台，支持身份识别和主数据管理。

- **Stars:** ⭐️ 1.2k
- **Tags:** `entity-resolution` `machine-learning` `deduplication` `data-quality`
- **最后活动时间:** 2026-03-14

### [pytorch-fm](https://github.com/rixwew/pytorch-fm)

PyTorch实现的因子分解机模型集合，支持DeepFM、NFM、DCN等多种CTR预测模型。

- **Stars:** ⭐️ 1.1k
- **Tags:** `factorization-machines` `ctr-prediction` `recommendation` `pytorch`
- **最后活动时间:** 2024-04-08

### [himarket](https://github.com/higress-group/himarket)

企业级AI能力市场与开发者生态中心，管理Model API、MCP服务器和Agent API。

- **Stars:** ⭐️ 1.0k
- **Tags:** `ai-marketplace` `enterprise` `mcp` `agent-api`
- **最后活动时间:** 2026-03-20

### [openops](https://github.com/openops-cloud/openops)

开源无代码FinOps自动化平台，集成可信AI能力，实现云成本优化与治理自动化。

- **Stars:** ⭐️ 1.0k
- **Tags:** `FinOps` `No-Code` `Cloud`
- **最后活动时间:** 2026-03-26

### [pycox](https://github.com/havakv/pycox)

基于PyTorch的生存分析库，提供多种深度学习生存模型实现。

- **Stars:** ⭐️ 966
- **Tags:** `survival-analysis` `deep-learning` `time-to-event` `pytorch`
- **最后活动时间:** 2024-09-04

### [deep-ctr-prediction](https://github.com/qiaoguan/deep-ctr-prediction)

基于深度学习的CTR预估模型集合，涵盖DeepFM、DIN、ESMM等主流推荐算法实现。

- **Stars:** ⭐️ 934
- **Tags:** `ctr-prediction` `recommendation-system` `deep-learning`
- **最后活动时间:** 2019-11-15

### [OpenDerisk](https://github.com/derisk-ai/OpenDerisk)

AI原生风险智能系统，提供7×24小时全面深入的应用系统风险监控与保护。

- **Stars:** ⭐️ 884
- **Tags:** `Risk Management` `Multi-Agent` `DevOps`
- **最后活动时间:** 2026-03-25

### [DISC-FinLLM](https://github.com/FudanDISC/DISC-FinLLM)

中文金融领域大语言模型，提供专业智能的金融咨询服务。

- **Stars:** ⭐️ 856
- **Tags:** `financial-llm` `chinese-llm` `rag` `instruction-tuning`
- **最后活动时间:** 2023-11-01

### [LLMs-in-Finance](https://github.com/hananedupouy/LLMs-in-Finance)

金融领域大语言模型应用合集，涵盖生成式AI和智能体在金融场景的实践。

- **Stars:** ⭐️ 850
- **Tags:** `Finance` `LLM` `AI Agents`
- **最后活动时间:** 2026-02-23

### [PIXIU](https://github.com/The-FinAI/PIXIU)

开源金融大语言模型项目，包含金融LLM、指令微调数据和评估基准。

- **Stars:** ⭐️ 839
- **Tags:** `financial-llm` `fintech` `large-language-models` `nlp`
- **最后活动时间:** 2025-03-04

### [chaos_genius](https://github.com/chaos-genius/chaos_genius)

基于机器学习的分析引擎，专注于异常检测和根因分析，帮助企业快速定位数据异常问题。

- **Stars:** ⭐️ 775
- **Tags:** `Anomaly Detection` `Analytics` `ML`
- **最后活动时间:** 2024-09-12

### [DGFraud](https://github.com/safe-graph/DGFraud)

基于图神经网络的欺诈检测深度学习工具箱，支持多种图算法和真实数据集。

- **Stars:** ⭐️ 750
- **Tags:** `Graph Neural Networks` `Fraud Detection` `Anomaly Detection`
- **最后活动时间:** 2022-04-20

### [WHartTest](https://github.com/MGdaasLab/WHartTest)

AI驱动的测试自动化平台，结合LangChain与MCP实现测试用例自动生成与管理。

- **Stars:** ⭐️ 737
- **Tags:** `test-automation` `langchain` `mcp` `enterprise`
- **最后活动时间:** 2026-03-25

### [innoshop](https://github.com/innocommerce/innoshop)

基于Laravel 12的AI驱动开源电商系统，支持多AI模型智能自动化、多语言和多货币。

- **Stars:** ⭐️ 608
- **Tags:** `ai-ecommerce` `laravel` `multi-language` `open-source`
- **最后活动时间:** 2026-03-24

### [0xDeCA10B](https://github.com/microsoft/0xDeCA10B)

基于区块链的可更新模型共享平台，支持AI模型去中心化部署与预测市场。

- **Stars:** ⭐️ 606
- **Tags:** `blockchain` `machine-learning` `smart-contracts` `decentralized-ai`
- **最后活动时间:** 2025-04-30

### [binance-skills-hub](https://github.com/binance/binance-skills-hub)

开放技能市场，为AI代理提供原生加密货币访问能力。连接AI代理与区块链金融服务。

- **Stars:** ⭐️ 567
- **Tags:** `crypto` `skills-marketplace` `ai-agents` `binance`
- **最后活动时间:** 2026-03-26

### [AI-Bank-Statement-Document-Automation-By-LLM-And-Personal-Finanical-Analysis-Prediction](https://github.com/johnsonhk88/AI-Bank-Statement-Document-Automation-By-LLM-And-Personal-Finanical-Analysis-Prediction)

基于LLM的银行账单文档自动化处理工具，结合RAG和OCR技术实现智能财务分析与预测。

- **Stars:** ⭐️ 564
- **Tags:** `LLM` `RAG` `OCR` `CrewAI` `LangChain`
- **最后活动时间:** 2026-03-22

### [SSLRec](https://github.com/HKUDS/SSLRec)

WSDM 2024自监督学习推荐系统框架。结合对比学习和图神经网络，提升推荐算法性能。

- **Stars:** ⭐️ 557
- **Tags:** `recommender-systems` `self-supervised-learning` `graph-neural-networks` `deep-learning`
- **最后活动时间:** 2025-03-21

### [echo](https://github.com/Merit-Systems/echo)

用户付费AI SDK，为AI应用提供计费和支付集成解决方案。

- **Stars:** ⭐️ 535
- **Tags:** `billing` `ai-sdk` `openai` `anthropic`
- **最后活动时间:** 2026-01-13

### [optbinning](https://github.com/guillermo-navas-palencia/optbinning)

最优分箱库，支持单调分箱和约束条件，适用于信用评分卡建模。提供反事实解释功能，支持批量和流式处理。

- **Stars:** ⭐️ 519
- **Tags:** `credit-scoring` `machine-learning` `optimization`
- **最后活动时间:** 2026-02-14

### [rever](https://github.com/makerever/rever)

AI驱动的财务自动化平台，为现代CFO提供智能化的财务控制、系统统一和前瞻性分析能力。

- **Stars:** ⭐️ 509
- **Tags:** `ai-finance` `automation` `invoice` `django`
- **最后活动时间:** 2026-03-24

## Market Analysis

### [OpenBB](https://github.com/OpenBB-finance/OpenBB)

面向分析师、量化交易者和AI代理的金融数据平台，支持股票、加密货币、衍生品等多类资产分析。

- **Stars:** ⭐️ 63.6k
- **Tags:** `finance` `quantitative-finance` `machine-learning` `ai`
- **最后活动时间:** 2026-03-26

### [daily_stock_analysis](https://github.com/ZhuLinsen/daily_stock_analysis)

LLM驱动的A股/港股/美股智能分析系统，整合多数据源行情与实时新闻，支持定时推送。

- **Stars:** ⭐️ 25.6k
- **Tags:** `Stock Analysis` `LLM` `Quant`
- **最后活动时间:** 2026-03-25

### [prophet](https://github.com/facebook/prophet)

Meta开源的时间序列预测工具，擅长处理多重季节性和非线性增长的数据预测。

- **Stars:** ⭐️ 20.1k
- **Tags:** `时间序列` `预测` `机器学习`
- **最后活动时间:** 2026-03-24

### [dexter](https://github.com/virattt/dexter)

专注于金融领域的自主研究代理，提供深度金融分析能力。

- **Stars:** ⭐️ 18.5k
- **Tags:** `Finance` `Agent` `Research`
- **最后活动时间:** 2026-03-25

### [akshare](https://github.com/akfamily/akshare)

优雅简洁的Python财经数据接口库，提供股票、期货、债券等金融数据的开源接口。

- **Stars:** ⭐️ 17.7k
- **Tags:** `finance` `financial-data` `quant` `stock` `python`
- **最后活动时间:** 2026-03-25

### [awesome-ai-in-finance](https://github.com/georgezouq/awesome-ai-in-finance)

精选AI在金融市场应用的资源列表，涵盖LLM、强化学习、量化交易等前沿策略与工具。

- **Stars:** ⭐️ 5.5k
- **Tags:** `Finance` `LLM` `Quantitative Trading` `Deep Learning`
- **最后活动时间:** 2026-03-25

### [go-stock](https://github.com/ArvinLovegood/go-stock)

AI赋能的股票分析工具，支持A股、港股、美股行情获取与AI情绪分析，数据本地存储，兼容多种AI平台。

- **Stars:** ⭐️ 5.1k
- **Tags:** `stock-analysis` `ai` `deepseek` `ollama` `openai` `golang`
- **最后活动时间:** 2026-03-26

### [NeuralProphet](https://github.com/ourownstory/neural_prophet)

基于神经网络的时间序列预测工具，继承Prophet优点并支持深度学习扩展。

- **Stars:** ⭐️ 4.3k
- **Tags:** `时间序列` `预测` `PyTorch`
- **最后活动时间:** 2025-01-08

### [alphalens](https://github.com/quantopian/alphalens)

用于分析预测性股票因子（Alpha）表现的专业工具，帮助量化研究员评估因子有效性和收益预测能力。

- **Stars:** ⭐️ 4.2k
- **Tags:** `algorithmic-trading` `finance` `quantitative-finance` `factor-analysis`
- **最后活动时间:** 2024-02-12

### [FinceptTerminal](https://github.com/Fincept-Corporation/FinceptTerminal)

现代化金融终端应用，集成机器学习功能提供高级市场分析、投资研究和经济数据工具，支持交互式数据探索。

- **Stars:** ⭐️ 2.9k
- **Tags:** `Finance` `Machine Learning` `Quantitative`
- **最后活动时间:** 2026-03-26

### [stocksight](https://github.com/shirosaidev/stocksight)

基于Elasticsearch、Twitter和新闻标题的股票市场分析与预测工具，使用NLP和情感分析技术。

- **Stars:** ⭐️ 2.4k
- **Tags:** `sentiment-analysis` `stock-prediction` `nlp` `elasticsearch`
- **最后活动时间:** 2023-12-05

### [stock-knowledge-graph](https://github.com/lemonhu/stock-knowledge-graph)

利用公开数据构建证券知识图谱，适合金融数据分析入门。

- **Stars:** ⭐️ 2.1k
- **Tags:** `knowledge-graph` `stock` `finance` `neo4j`
- **最后活动时间:** 2020-07-23

### [ai-financial-agent](https://github.com/virattt/ai-financial-agent)

AI金融投资研究智能体，辅助投资决策和市场分析。

- **Stars:** ⭐️ 1.9k
- **Tags:** `financial-agent` `investment` `market-analysis`
- **最后活动时间:** 2025-08-19

### [financial-datasets-mcp](https://github.com/financial-datasets/mcp-server)

对接Financial Datasets股票市场API的MCP服务器，为AI代理提供实时金融数据访问能力。

- **Stars:** ⭐️ 1.7k
- **Tags:** `mcp` `finance` `stock-market` `trading`
- **最后活动时间:** 2025-06-05

### [MarS](https://github.com/microsoft/MarS)

基于生成式基础模型的金融市场模拟引擎，用于金融市场建模与预测。

- **Stars:** ⭐️ 1.7k
- **Tags:** `financial-simulation` `generative-model` `market-analysis`
- **最后活动时间:** 2025-08-06

### [deep-finance](https://github.com/sangyx/deep-finance)

AI与金融领域的综合资源合集，包含数据集、论文和书籍推荐。

- **Stars:** ⭐️ 1.5k
- **Tags:** `ai-finance` `deep-learning` `fintech`
- **最后活动时间:** 2022-06-28

### [lifetimes](https://github.com/CamDavidsonPilon/lifetimes)

客户终身价值分析Python库，用于预测客户行为、购买频率和留存率建模。

- **Stars:** ⭐️ 1.5k
- **Tags:** `customer-analytics` `lifetime-value` `statistics` `python`
- **最后活动时间:** 2024-06-28

### [stockbot-on-groq](https://github.com/bklieger-groq/stockbot-on-groq)

基于Groq和Llama3-70b的超快AI股票分析聊天机器人，支持实时交互式图表、财务数据和新闻分析。

- **Stars:** ⭐️ 1.5k
- **Tags:** `Groq` `Llama3` `Stock Analysis` `Chatbot`
- **最后活动时间:** 2025-12-30

### [FinNLP](https://github.com/AI4Finance-Foundation/FinNLP)

金融数据获取与处理库，致力于民主化互联网规模的金融数据访问。

- **Stars:** ⭐️ 1.4k
- **Tags:** `finance` `nlp` `data`
- **最后活动时间:** 2024-07-01

### [pytorch-ts](https://github.com/zalandoresearch/pytorch-ts)

基于PyTorch的概率时间序列预测框架，集成DeepAR、N-BEATS等模型。

- **Stars:** ⭐️ 1.4k
- **Tags:** `time-series` `forecasting` `deepar` `n-beats` `pytorch`
- **最后活动时间:** 2024-06-14

### [experimental-openbb-platform-agent](https://github.com/OpenBB-finance/experimental-openbb-platform-agent)

基于OpenBB平台的AI智能体实验项目，用于金融数据分析和投资研究。

- **Stars:** ⭐️ 1.3k
- **Tags:** `financial-agent` `openbb` `investment-analysis`
- **最后活动时间:** 2024-07-22

### [pymc-marketing](https://github.com/pymc-labs/pymc-marketing)

基于PyMC的贝叶斯营销工具箱，提供媒体组合模型(MMM)、客户生命周期价值(CLV)等营销分析模型。

- **Stars:** ⭐️ 1.1k
- **Tags:** `marketing` `bayesian` `mmm` `clv` `python`
- **最后活动时间:** 2026-03-26

### [lightweight_mmm](https://github.com/google/lightweight_mmm)

轻量级贝叶斯营销组合模型库，帮助用户轻松训练MMM模型并获取渠道归因分析，支持营销效果量化评估。

- **Stars:** ⭐️ 1.0k
- **Tags:** `bayesian` `marketing-mix-model` `econometrics` `attribution` `marketing-science`
- **最后活动时间:** 2025-06-17

### [Sentiment-Analysis-in-Event-Driven-Stock-Price-Movement-Prediction](https://github.com/WayneDW/Sentiment-Analysis-in-Event-Driven-Stock-Price-Movement-Prediction)

利用NLP情感分析预测新闻事件驱动的股价走势，结合事件驱动策略进行量化分析。

- **Stars:** ⭐️ 877
- **Tags:** `sentiment-analysis` `stock-prediction` `nlp`
- **最后活动时间:** 2024-07-15

### [Stock-Market-Prediction-Web-App-using-Machine-Learning-And-Sentiment-Analysis](https://github.com/kaushikjadhav01/Stock-Market-Prediction-Web-App-using-Machine-Learning-And-Sentiment-Analysis)

基于机器学习和情感分析的股票市场预测Web应用，支持ARIMA、LSTM、线性回归三种算法预测未来七天股价，结合Twitter情感分析提供涨跌建议。

- **Stars:** ⭐️ 873
- **Tags:** `machine-learning` `lstm` `sentiment-analysis` `stock-prediction` `tensorflow`
- **最后活动时间:** 2024-02-19

### [local-llms-analyse-finance](https://github.com/thu-vu92/local-llms-analyse-finance)

使用本地LLM（如Llama2）自动分类银行交易数据，探索本地大模型在金融数据分析中的应用。

- **Stars:** ⭐️ 865
- **Tags:** `local-llm` `finance` `data-labeling` `llama2`
- **最后活动时间:** 2024-05-16

### [jcp](https://github.com/run-bigpig/jcp)

基于Wails+Go+React构建的AI驱动A股智能分析系统，支持多Agent协作进行市场分析。

- **Stars:** ⭐️ 838
- **Tags:** `stock-analysis` `multi-agent` `ai-finance` `wails`
- **最后活动时间:** 2026-03-03

### [finance](https://github.com/yorkeccak/finance)

开源金融AI助手，通过自然对话获取机构级金融数据、运行复杂分析并创建可视化图表。

- **Stars:** ⭐️ 815
- **Tags:** `ai-assistant` `financial-analysis` `natural-language` `visualization`
- **最后活动时间:** 2026-03-16

### [investbrain](https://github.com/investbrainapp/investbrain)

基于LLM的智能投资组合追踪器，支持跨券商整合市场数据并监控投资表现。

- **Stars:** ⭐️ 805
- **Tags:** `Investment` `LLM` `Self-hosted`
- **最后活动时间:** 2026-03-25

### [stocks-insights-ai-agent](https://github.com/vinay-gatech/stocks-insights-ai-agent)

全栈股票数据与新闻AI分析应用，结合LLM、LangChain与LangGraph。

- **Stars:** ⭐️ 737
- **Tags:** `stocks` `finance` `langchain` `langgraph` `ai-agent`
- **最后活动时间:** 2024-12-08

### [opennews-mcp](https://github.com/6551Team/opennews-mcp)

加密货币新闻聚合平台，集成AI评级和交易信号功能，提供实时市场更新。

- **Stars:** ⭐️ 704
- **Tags:** `Crypto` `AI Ratings` `Trading Signals` `News Aggregation`
- **最后活动时间:** 2026-03-11

### [sports-betting](https://github.com/georgedouzas/sports-betting)

体育博彩AI工具集合，包含机器学习预测模型和数据分析工具。

- **Stars:** ⭐️ 678
- **Tags:** `sports-betting` `machine-learning` `sports-analytics`
- **最后活动时间:** 2026-01-21

### [stocknet-dataset](https://github.com/yumoxu/stocknet-dataset)

股票走势预测综合数据集，整合推文情感和历史股价数据，适用于金融量化研究。

- **Stars:** ⭐️ 674
- **Tags:** `stock-prediction` `dataset` `finance`
- **最后活动时间:** 2019-03-06

### [FinBERT](https://github.com/yya518/FinBERT)

面向金融领域的预训练BERT模型，专用于金融文本情感分析和金融通信理解。

- **Stars:** ⭐️ 649
- **Tags:** `bert` `financial-sentiment-analysis` `nlp`
- **最后活动时间:** 2023-07-23

### [trump-code](https://github.com/sstklen/trump-code)

开源AI项目，通过分析特朗普社交媒体帖子预测美股走势，模型命中率高达61.3%，包含3150万模型参数。

- **Stars:** ⭐️ 649
- **Tags:** `ai` `machine-learning` `stock-market` `prediction` `signal-analysis`
- **最后活动时间:** 2026-03-25

### [Deep-Convolution-Stock-Technical-Analysis](https://github.com/philipxjm/Deep-Convolution-Stock-Technical-Analysis)

利用深度卷积神经网络进行股市技术分析，预测股票未来走势。

- **Stars:** ⭐️ 624
- **Tags:** `cnn` `stock-market` `technical-analysis` `price-prediction`
- **最后活动时间:** 2023-03-25

### [FX-1-Minute-Data](https://github.com/philipperemy/FX-1-Minute-Data)

外汇、原油和股指的1分钟级别历史数据集，提供简单API获取历史价格数据，适用于金融机器学习和深度学习研究。

- **Stars:** ⭐️ 616
- **Tags:** `dataset` `financial-data` `trading` `machine-learning`
- **最后活动时间:** 2025-05-04

### [cryptoCMD](https://github.com/guptarohit/cryptoCMD)

加密货币历史价格数据Python库，从CoinMarketCap获取数据，适用于量化交易和机器学习研究。

- **Stars:** ⭐️ 602
- **Tags:** `cryptocurrency` `historical-data` `python` `trading`
- **最后活动时间:** 2026-01-13

### [awesome-data](https://github.com/akfamily/awesome-data)

金融领域数据源精选列表，汇集全球金融市场最有价值的数据资源。

- **Stars:** ⭐️ 589
- **Tags:** `finance` `data-sources` `financial-data` `awesome-list`
- **最后活动时间:** 2025-03-27

### [TradingView-data-scraper](https://github.com/jchao01/TradingView-data-scraper)

从TradingView图表提取价格和指标数据，用于创建机器学习数据集。

- **Stars:** ⭐️ 556
- **Tags:** `trading` `data-extraction` `webscraping` `ml-datasets`
- **最后活动时间:** 2022-07-06

### [borsa-mcp](https://github.com/saidsurucu/borsa-mcp)

土耳其和美国证券交易所及基金数据的MCP服务器，为AI模型提供金融市场数据访问能力。

- **Stars:** ⭐️ 533
- **Tags:** `mcp` `mcp-server` `stock-exchange` `financial-data` `market-analysis`
- **最后活动时间:** 2026-03-16

### [defeatbeta-api](https://github.com/defeat-beta/defeatbeta-api)

开源的雅虎财经API替代方案，集成AI交易分析和财报电话会议GPT转录功能，提供更可靠的金融市场数据服务。

- **Stars:** ⭐️ 522
- **Tags:** `ai-trading` `financial-data` `market-data` `stock-analysis`
- **最后活动时间:** 2026-03-19

### [FinanceMCP](https://github.com/guangxiangdebizi/FinanceMCP)

为LLM提供金融数据访问能力的MCP服务器，支持股票、基金、加密货币等多维度金融数据分析。

- **Stars:** ⭐️ 514
- **Tags:** `mcp` `mcp-server` `llm` `finance` `data`
- **最后活动时间:** 2026-03-04

## Quantitative Trading

### [ai-hedge-fund](https://github.com/virattt/ai-hedge-fund)

AI驱动的对冲基金团队，利用多智能体协作进行量化交易决策和市场分析。

- **Stars:** ⭐️ 49.6k
- **Tags:** `ai-trading` `hedge-fund` `quantitative` `multi-agent`
- **最后活动时间:** 2026-03-19

### [freqtrade](https://github.com/freqtrade/freqtrade)

开源加密货币量化交易机器人，支持策略回测、技术指标分析和自动化交易。

- **Stars:** ⭐️ 48.1k
- **Tags:** `trading-bot` `cryptocurrency` `algorithmic-trading` `python`
- **最后活动时间:** 2026-03-26

### [TradingAgents](https://github.com/TauricResearch/TradingAgents)

基于多智能体LLM的金融交易框架，利用多个AI智能体协同进行金融交易决策与分析。

- **Stars:** ⭐️ 42.1k
- **Tags:** `Multi-Agent` `LLM` `Quantitative Trading`
- **最后活动时间:** 2026-03-22

### [qlib](https://github.com/microsoft/qlib)

微软开源的AI量化投资平台，覆盖量化研究全流程，支持多种机器学习建模范式。

- **Stars:** ⭐️ 39.3k
- **Tags:** `量化投资` `机器学习` `深度学习`
- **最后活动时间:** 2026-03-10

### [vnpy](https://github.com/vnpy/vnpy)

基于Python的开源量化交易平台框架，支持多种交易接口和策略开发。

- **Stars:** ⭐️ 38.4k
- **Tags:** `quantitative-trading` `algorithmic-trading` `python` `fintech`
- **最后活动时间:** 2026-01-14

### [awesome-quant](https://github.com/wilsonfreitas/awesome-quant)

量化金融领域精选资源列表，涵盖算法交易、交易机器人、技术分析等工具和库，适合量化交易开发者参考。

- **Stars:** ⭐️ 25.1k
- **Tags:** `quantitative-finance` `algorithmic-trading` `trading-bot`
- **最后活动时间:** 2026-03-26

### [nautilus_trader](https://github.com/nautechsystems/nautilus_trader)

生产级Rust原生高性能交易引擎，支持确定性事件驱动架构，集成机器学习功能，覆盖加密货币、股票、外汇等多市场。

- **Stars:** ⭐️ 21.4k
- **Tags:** `Quantitative Trading` `Machine Learning` `Rust`
- **最后活动时间:** 2026-03-26

### [TradingAgents-CN](https://github.com/hsliuping/TradingAgents-CN)

基于多智能体LLM的中文金融交易框架，支持多角色协作进行市场分析和交易决策。

- **Stars:** ⭐️ 21.3k
- **Tags:** `Multi-Agent` `LLM` `Trading` `Finance`
- **最后活动时间:** 2026-02-14

### [Lean](https://github.com/QuantConnect/Lean)

QuantConnect开源的算法交易引擎，支持Python和C#，提供完整的回测和实盘交易能力。

- **Stars:** ⭐️ 18.1k
- **Tags:** `量化交易` `算法交易` `金融`
- **最后活动时间:** 2026-03-25

### [hummingbot](https://github.com/hummingbot/hummingbot)

开源加密货币高频交易机器人框架，支持创建和部署自动化交易策略，包含套利、做市等功能。

- **Stars:** ⭐️ 17.8k
- **Tags:** `Trading Bot` `Cryptocurrency` `Algorithmic Trading`
- **最后活动时间:** 2026-03-24

### [machine-learning-for-trading](https://github.com/stefan-jansen/machine-learning-for-trading)

机器学习算法交易实战代码，涵盖投资策略、交易代理和合成数据等内容。量化交易学习必备资源。

- **Stars:** ⭐️ 16.8k
- **Tags:** `machine-learning` `trading` `finance`
- **最后活动时间:** 2024-08-18

### [Qbot](https://github.com/UFund-Me/Qbot)

AI驱动的量化投资研究平台，支持完全本地部署的自动量化交易机器人。集成深度学习与机器学习技术，提供回测、策略研究与实盘交易功能。

- **Stars:** ⭐️ 16.7k
- **Tags:** `量化交易` `机器学习` `金融科技`
- **最后活动时间:** 2026-03-11

### [abu](https://github.com/bbfamily/abu)

基于Python的开源量化交易系统，支持多种交易品种，集成机器学习功能，提供完整量化投资架构。

- **Stars:** ⭐️ 16.6k
- **Tags:** `量化交易` `机器学习` `比特币`
- **最后活动时间:** 2026-01-24

### [FinRL](https://github.com/AI4Finance-Foundation/FinRL)

金融强化学习框架，提供深度强化学习算法在量化交易中的应用，支持多种交易任务和市场环境。

- **Stars:** ⭐️ 14.5k
- **Tags:** `deep-reinforcement-learning` `algorithmic-trading` `stock-trading` `fintech`
- **最后活动时间:** 2026-03-24

### [AI-Trader](https://github.com/HKUDS/AI-Trader)

AI量化交易研究项目，探索AI能否战胜市场，提供实盘交易基准测试与技术报告。

- **Stars:** ⭐️ 11.9k
- **Tags:** `ai-trading` `quantitative-finance` `algorithmic-trading`
- **最后活动时间:** 2026-03-23

### [nofx](https://github.com/NoFxAiOS/nofx)

个人AI交易助手，支持多市场多模型，通过USDC支付而非API密钥进行交易。

- **Stars:** ⭐️ 11.2k
- **Tags:** `AI Trading` `Agentic Trading` `Multi-market`
- **最后活动时间:** 2026-03-25

### [quant-trading](https://github.com/je-suis-tm/quant-trading)

Python量化交易策略合集，包含VIX计算器、模式识别、蒙特卡洛模拟、期权策略、配对交易、RSI、布林带等多种算法交易策略。

- **Stars:** ⭐️ 9.5k
- **Tags:** `algorithmic-trading` `quantitative-trading` `trading-strategies` `python`
- **最后活动时间:** 2024-04-14

### [StockSharp](https://github.com/StockSharp/StockSharp)

开源算法交易平台，支持股票、外汇、加密货币的量化交易策略开发与回测。

- **Stars:** ⭐️ 9.3k
- **Tags:** `algorithmic-trading` `quantitative-finance` `trading`
- **最后活动时间:** 2026-03-24

### [Stock-Prediction-Models](https://github.com/huseinzol05/Stock-Prediction-Models)

股票预测机器学习与深度学习模型合集，包含交易机器人与策略模拟。

- **Stars:** ⭐️ 9.3k
- **Tags:** `stock-prediction` `deep-learning` `trading-bot` `lstm`
- **最后活动时间:** 2023-04-16

### [backtesting.py](https://github.com/kernc/backtesting.py)

Python量化交易回测框架，支持快速验证和优化交易策略，适用于算法交易和金融研究。

- **Stars:** ⭐️ 8.1k
- **Tags:** `backtesting` `algorithmic-trading` `trading-strategies` `finance`
- **最后活动时间:** 2025-12-20

### [jesse](https://github.com/jesse-ai/jesse)

用Python编写的高级加密货币交易机器人框架，支持量化交易策略开发和回测。

- **Stars:** ⭐️ 7.6k
- **Tags:** `trading-bot` `crypto` `algorithmic-trading` `quantitative-finance`
- **最后活动时间:** 2026-03-22

### [LLM-Trading-Lab](https://github.com/LuckyOne7777/LLM-Trading-Lab)

一个使用ChatGPT管理实盘微型股票投资组合的实验项目，探索LLM在金融交易决策中的应用。

- **Stars:** ⭐️ 7.4k
- **Tags:** `LLM` `Trading` `ChatGPT` `Finance`
- **最后活动时间:** 2026-03-22

### [vectorbt](https://github.com/polakowo/vectorbt)

超高速回测引擎，支持量化交易策略开发与优化，集成机器学习功能。

- **Stars:** ⭐️ 7.0k
- **Tags:** `Quantitative Trading` `Backtesting` `Machine Learning`
- **最后活动时间:** 2026-03-19

### [Financial-Models-Numerical-Methods](https://github.com/cantaro86/Financial-Models-Numerical-Methods)

量化金融笔记本合集，包含期权定价、蒙特卡洛方法、随机微分方程等交互式Python代码。

- **Stars:** ⭐️ 6.7k
- **Tags:** `quantitative-finance` `option-pricing` `monte-carlo` `financial-engineering`
- **最后活动时间:** 2024-10-22

### [TensorTrade](https://github.com/tensortrade-org/tensortrade)

开源强化学习框架，用于训练、评估和部署稳健的交易智能体，支持多种RL算法。

- **Stars:** ⭐️ 6.1k
- **Tags:** `Reinforcement Learning` `Trading` `AI Framework` `Quantitative`
- **最后活动时间:** 2026-02-19

### [PyPortfolioOpt](https://github.com/PyPortfolio/PyPortfolioOpt)

Python金融投资组合优化库，支持经典有效前沿、Black-Litterman模型和层次风险平价等优化策略。

- **Stars:** ⭐️ 5.6k
- **Tags:** `portfolio-optimization` `quantitative-finance` `python`
- **最后活动时间:** 2026-03-10

### [OctoBot](https://github.com/Drakkar-Software/OctoBot)

开源加密货币交易机器人，支持AI策略、网格交易、DCA及TradingView策略自动化。

- **Stars:** ⭐️ 5.5k
- **Tags:** `ai-trading` `trading-bot` `cryptocurrency` `backtest`
- **最后活动时间:** 2026-03-25

### [stockpredictionai](https://github.com/borisbanushev/stockpredictionai)

使用GAN、LSTM和CNN进行股票价格预测的完整项目，结合生成对抗网络与时序模型实现金融预测。

- **Stars:** ⭐️ 5.5k
- **Tags:** `stock-prediction` `gan` `lstm` `cnn` `time-series`
- **最后活动时间:** 2025-08-19

### [binance-trading-bot](https://github.com/chrisleekr/binance-trading-bot)

自动化加密货币交易机器人，支持网格交易策略，集成TradingView技术分析指标，实现低买高卖的自动交易。

- **Stars:** ⭐️ 5.5k
- **Tags:** `Trading Bot` `Cryptocurrency` `Grid Trading`
- **最后活动时间:** 2025-06-10

### [Superalgos](https://github.com/Superalgos/Superalgos)

免费开源的加密货币量化交易平台，支持可视化策略设计、回测、纸面交易和多服务器部署。

- **Stars:** ⭐️ 5.4k
- **Tags:** `algorithmic-trading` `crypto-trading` `trading-bot`
- **最后活动时间:** 2026-03-24

### [ai_quant_trade](https://github.com/charliedream1/ai_quant_trade)

股票AI操盘手一站式平台，涵盖机器学习、深度学习、强化学习、图网络等AI技术在量化交易中的完整应用，包含策略实例与实盘代码。

- **Stars:** ⭐️ 5.4k
- **Tags:** `quantitative-trading` `machine-learning` `deep-learning` `reinforcement-learning` `trading-bot`
- **最后活动时间:** 2026-03-18

### [tf-quant-finance](https://github.com/google/tf-quant-finance)

基于TensorFlow的高性能量化金融库，提供GPU加速的金融数值计算、期权定价和风险管理工具。

- **Stars:** ⭐️ 5.3k
- **Tags:** `tensorflow` `quantitative-finance` `gpu` `derivatives` `pricing`
- **最后活动时间:** 2026-02-12

### [awesome-quant](https://github.com/thuquant/awesome-quant)

中国量化投资相关资源索引，涵盖策略、工具和学习资料。

- **Stars:** ⭐️ 5.1k
- **Tags:** `quant` `finance` `trading` `python` `machine-learning`
- **最后活动时间:** 2026-03-23

### [ta](https://github.com/bukosabino/ta)

基于Pandas和Numpy的技术分析库，提供58种以上金融技术指标，适用于量化交易和金融市场分析。

- **Stars:** ⭐️ 4.9k
- **Tags:** `technical-analysis` `trading` `financial` `pandas` `numpy`
- **最后活动时间:** 2026-03-18

### [mlfinlab](https://github.com/hudson-and-thames/mlfinlab)

面向投资组合经理和交易员的机器学习工具库，提供可复现、可解释的量化金融解决方案。

- **Stars:** ⭐️ 4.6k
- **Tags:** `Quantitative Finance` `Algorithmic Trading` `Portfolio`
- **最后活动时间:** 2023-10-02

### [tqsdk-python](https://github.com/shinnytech/tqsdk-python)

天勤量化开发包，支持期货量化交易、实时行情获取、历史数据分析和实盘交易。

- **Stars:** ⭐️ 4.6k
- **Tags:** `quant` `futures` `python` `trading`
- **最后活动时间:** 2026-03-18

### [adata](https://github.com/1nchaos/adata)

免费开源的A股量化交易数据库，为个人量化交易和AI研究提供高质量数据支持。

- **Stars:** ⭐️ 4.2k
- **Tags:** `stock-data` `quantitative-trading` `financial-data` `a-share`
- **最后活动时间:** 2025-12-26

### [zvt](https://github.com/zvtvz/zvt)

模块化量化交易框架，支持数字货币和股票的回测与实盘交易。

- **Stars:** ⭐️ 4.0k
- **Tags:** `quantitative-trading` `algorithmic-trading` `backtesting` `trading-bot`
- **最后活动时间:** 2026-03-07

### [neuralforecast](https://github.com/Nixtla/neuralforecast)

可扩展的神经网络时间序列预测库，支持 N-BEATS、NHITS、TFT 等先进模型，提供概率和分层预测。

- **Stars:** ⭐️ 4.0k
- **Tags:** `Time Series` `Forecasting` `PyTorch` `Deep Learning`
- **最后活动时间:** 2026-03-26

### [hftbacktest](https://github.com/nkaz001/hftbacktest)

开源高频交易回测引擎，支持限价订单、队列位置和延迟模拟，提供Binance和Bybit实盘示例。

- **Stars:** ⭐️ 3.8k
- **Tags:** `algorithmic-trading` `backtesting` `hft` `market-making` `crypto-trading`
- **最后活动时间:** 2025-12-23

### [EliteQuant](https://github.com/EliteQuant/EliteQuant)

量化建模、交易和投资组合管理的在线资源汇总列表。

- **Stars:** ⭐️ 3.8k
- **Tags:** `quantitative-trading` `algorithmic-trading` `machine-learning` `portfolio-management`
- **最后活动时间:** 2024-06-15

### [Finance](https://github.com/shashankvemuri/Finance)

150+量化金融Python程序集合，涵盖机器学习驱动的股票分析、交易策略和技术指标计算。

- **Stars:** ⭐️ 3.7k
- **Tags:** `Quantitative Finance` `Machine Learning` `Trading`
- **最后活动时间:** 2026-03-26

### [Awesome-Quant-Machine-Learning-Trading](https://github.com/grananqvist/Awesome-Quant-Machine-Learning-Trading)

量化与算法交易资源汇总，重点关注机器学习在交易中的应用。

- **Stars:** ⭐️ 3.5k
- **Tags:** `awesome-list` `quantitative-trading` `machine-learning` `financial-machine-learning`
- **最后活动时间:** 2025-05-21

### [pybroker](https://github.com/edtechre/pybroker)

基于机器学习的Python算法交易框架，支持回测和策略开发。适合量化交易者和金融数据科学研究。

- **Stars:** ⭐️ 3.2k
- **Tags:** `Machine Learning` `Algorithmic Trading` `Backtesting`
- **最后活动时间:** 2026-03-05

### [pysystemtrade](https://github.com/pst-group/pysystemtrade)

Python系统化交易框架，提供完整的策略回测与实盘交易功能。

- **Stars:** ⭐️ 3.2k
- **Tags:** `trading` `backtesting` `quantitative-finance` `python`
- **最后活动时间:** 2026-03-23

### [eiten](https://github.com/tradytics/eiten)

面向所有人的统计与算法投资策略工具。结合机器学习与遗传算法进行投资组合优化和交易策略开发。

- **Stars:** ⭐️ 3.2k
- **Tags:** `algorithmic-trading` `machine-learning` `portfolio-optimization` `genetic-algorithm`
- **最后活动时间:** 2022-07-30

### [hikyuu](https://github.com/fasiondog/hikyuu)

基于C++/Python的高性能量化交易研究框架，支持策略回测、资产管理和快速策略迭代。

- **Stars:** ⭐️ 3.1k
- **Tags:** `Quantitative Trading` `Backtesting` `Finance`
- **最后活动时间:** 2026-03-25

### [DataFrame](https://github.com/hosseinmoein/DataFrame)

现代C++实现的DataFrame库，专为统计分析、金融数据和机器学习分析而设计。

- **Stars:** ⭐️ 2.9k
- **Tags:** `cpp` `dataframe` `data-analysis` `machine-learning` `financial-analysis`
- **最后活动时间:** 2026-03-23

### [OpenAlice](https://github.com/TraderAlice/OpenAlice)

文件驱动的AI交易智能体引擎，支持加密货币和证券市场。

- **Stars:** ⭐️ 2.9k
- **Tags:** `trading-agent` `crypto` `quantitative`
- **最后活动时间:** 2026-03-26

### [QuantResearch](https://github.com/letianzj/QuantResearch)

量化分析、策略研究和回测项目集合，涵盖机器学习在金融领域的多种应用。

- **Stars:** ⭐️ 2.9k
- **Tags:** `quantitative-trading` `backtesting` `machine-learning` `trading-strategies`
- **最后活动时间:** 2023-08-26

### [algorithmic-trading-python](https://github.com/nickmccullum/algorithmic-trading-python)

freeCodeCamp算法交易课程代码，涵盖Python量化交易策略与实践。

- **Stars:** ⭐️ 2.8k
- **Tags:** `algorithmic-trading` `quantitative-finance` `python`
- **最后活动时间:** 2024-06-20

### [FinRL-Trading](https://github.com/AI4Finance-Foundation/FinRL-Trading)

AI原生模块化量化交易基础设施，支持多种深度强化学习算法用于自动化股票交易策略开发。

- **Stars:** ⭐️ 2.8k
- **Tags:** `深度强化学习` `量化交易` `股票`
- **最后活动时间:** 2026-03-18

### [agents](https://github.com/Polymarket/agents)

基于AI智能体的Polymarket预测市场自主交易系统，实现自动化交易策略执行。

- **Stars:** ⭐️ 2.6k
- **Tags:** `trading-agent` `polymarket` `prediction-market` `autonomous-trading`
- **最后活动时间:** 2024-11-05

### [TradeMaster](https://github.com/TradeMaster-NTU/TradeMaster)

基于强化学习的开源量化交易平台，提供完整的交易策略开发工具链。

- **Stars:** ⭐️ 2.5k
- **Tags:** `quantitative-trading` `reinforcement-learning` `trading-platform` `pytorch`
- **最后活动时间:** 2025-06-04

### [xalpha](https://github.com/refraction-ray/xalpha)

支持AI智能体的基金投资管理回测引擎，提供量化交易策略分析与回测功能。

- **Stars:** ⭐️ 2.4k
- **Tags:** `ai-agents` `backtest` `quantitative-trading` `investment`
- **最后活动时间:** 2026-03-18

### [gym-anytrading](https://github.com/AminHP/gym-anytrading)

简洁灵活的OpenAI Gym交易环境，支持DQN、Q-Learning等强化学习算法进行股票和外汇交易策略研究。

- **Stars:** ⭐️ 2.4k
- **Tags:** `Reinforcement Learning` `Trading` `OpenAI Gym`
- **最后活动时间:** 2024-03-14

### [bulbea](https://github.com/achillesrasquinha/bulbea)

基于深度学习的股票市场预测与建模Python库，支持情感分析和量化交易策略开发。

- **Stars:** ⭐️ 2.3k
- **Tags:** `deep-learning` `stock-prediction` `quantitative-trading` `finance`
- **最后活动时间:** 2021-01-17

### [SGX-Full-OrderBook-Tick-Data-Trading-Strategy](https://github.com/rorysroes/SGX-Full-OrderBook-Tick-Data-Trading-Strategy)

基于完整订单簿tick数据的高频交易策略解决方案，运用机器学习进行特征工程与模型选择。

- **Stars:** ⭐️ 2.2k
- **Tags:** `high-frequency-trading` `orderbook` `quantitative-trading`
- **最后活动时间:** 2022-08-27

### [QuantMuse](https://github.com/0xemmkty/QuantMuse)

AI驱动的量化交易系统，集成实时数据处理、智能分析与高级风险管理功能。

- **Stars:** ⭐️ 2.1k
- **Tags:** `quantitative-trading` `machine-learning` `trading-system` `risk-management`
- **最后活动时间:** 2025-07-29

### [pycryptobot](https://github.com/whittlem/pycryptobot)

Python加密货币交易机器人，支持回测、技术分析和多交易所API集成。

- **Stars:** ⭐️ 2.1k
- **Tags:** `trading-bot` `cryptocurrency` `python` `backtesting`
- **最后活动时间:** 2026-03-26

### [MachineLearningStocks](https://github.com/robertmartin8/MachineLearningStocks)

使用Python和scikit-learn进行股票预测的入门教程和实践指南。

- **Stars:** ⭐️ 1.9k
- **Tags:** `stock-prediction` `machine-learning` `scikit-learn` `trading`
- **最后活动时间:** 2024-06-17

### [AlphaGPT](https://github.com/imbue-bit/AlphaGPT)

使用符号回归和深度学习在中国股市与加密市场进行高效因子挖掘的AI工具。

- **Stars:** ⭐️ 1.9k
- **Tags:** `deep-learning` `quant` `transformer` `finance`
- **最后活动时间:** 2026-02-24

### [AIAlpha](https://github.com/VivekPa/AIAlpha)

使用无监督和监督学习方法预测股票价格的AI量化项目，结合LSTM、自编码器和小波变换技术。

- **Stars:** ⭐️ 1.9k
- **Tags:** `Stock Prediction` `LSTM` `Quantitative Finance`
- **最后活动时间:** 2020-06-18

### [skfolio](https://github.com/skfolio/skfolio)

基于scikit-learn构建的投资组合优化Python库，支持多种风险模型与优化策略。

- **Stars:** ⭐️ 1.9k
- **Tags:** `portfolio-optimization` `quantitative-finance` `scikit-learn`
- **最后活动时间:** 2026-03-22

### [Adv_Fin_ML_Exercises](https://github.com/BlackArbsCEO/Adv_Fin_ML_Exercises)

《Advances in Financial Machine Learning》书籍精选习题的实验性解答，适合金融量化学习。

- **Stars:** ⭐️ 1.9k
- **Tags:** `financial-machine-learning` `quantitative-finance` `algorithms`
- **最后活动时间:** 2022-12-08

### [surpriver](https://github.com/tradytics/surpriver)

利用机器学习和异常检测技术发现大波动股票的量化交易工具。

- **Stars:** ⭐️ 1.9k
- **Tags:** `Algorithmic Trading` `Machine Learning` `Stock Analysis`
- **最后活动时间:** 2021-08-13

### [TradingGym](https://github.com/Yvictor/TradingGym)

专为强化学习代理设计的交易与回测环境，支持策略训练和简单规则算法验证。

- **Stars:** ⭐️ 1.9k
- **Tags:** `trading` `reinforcement-learning` `backtesting` `gym-environment`
- **最后活动时间:** 2024-02-11

### [PGPortfolio](https://github.com/ZhengyaoJiang/PGPortfolio)

基于深度强化学习的投资组合管理系统，使用策略梯度方法优化金融资产配置。

- **Stars:** ⭐️ 1.9k
- **Tags:** `deep-reinforcement-learning` `portfolio-management` `quantitative-finance`
- **最后活动时间:** 2021-10-09

### [RLTrader](https://github.com/notadamking/RLTrader)

基于深度强化学习的加密货币交易环境，使用 OpenAI Gym 构建智能交易策略。

- **Stars:** ⭐️ 1.9k
- **Tags:** `reinforcement-learning` `trading` `cryptocurrency` `gym`
- **最后活动时间:** 2022-01-22

### [awesome-deep-trading](https://github.com/cbailes/awesome-deep-trading)

机器学习算法交易精选资源列表，涵盖深度学习、强化学习在量化交易中的应用。

- **Stars:** ⭐️ 1.8k
- **Tags:** `量化交易` `深度学习` `资源列表`
- **最后活动时间:** 2023-08-08

### [FinRL-Meta](https://github.com/AI4Finance-Foundation/FinRL-Meta)

基于深度强化学习的金融AI框架，提供动态数据集和市场环境，支持量化交易智能体训练与回测。

- **Stars:** ⭐️ 1.8k
- **Tags:** `deep-reinforcement-learning` `drl-trading` `finrl` `openai-gym`
- **最后活动时间:** 2026-03-25

### [CausalImpact](https://github.com/google/CausalImpact)

R语言因果推断包，用于时间序列数据的干预效果分析和因果评估。

- **Stars:** ⭐️ 1.8k
- **Tags:** `causal-inference` `time-series` `r` `statistics`
- **最后活动时间:** 2025-09-26

### [Hands-On-Machine-Learning-for-Algorithmic-Trading](https://github.com/PacktPublishing/Hands-On-Machine-Learning-for-Algorithmic-Trading)

《算法交易机器学习实战》配套代码，讲解ML在量化交易中的应用。

- **Stars:** ⭐️ 1.8k
- **Tags:** `algorithmic-trading` `machine-learning` `finance`
- **最后活动时间:** 2023-01-18

### [fastquant](https://github.com/enzoampil/fastquant)

三行代码实现机器学习交易策略的回测与优化。

- **Stars:** ⭐️ 1.7k
- **Tags:** `quantitative-trading` `backtesting` `machine-learning`
- **最后活动时间:** 2023-09-15

### [machine-learning-asset-management](https://github.com/firmai/machine-learning-asset-management)

资产管理中的机器学习应用合集，涵盖投资组合优化、交易策略等量化金融场景。

- **Stars:** ⭐️ 1.7k
- **Tags:** `asset-management` `portfolio-optimization` `quantitative-finance` `trading-strategies`
- **最后活动时间:** 2021-12-17

### [Deep_Learning_Machine_Learning_Stock](https://github.com/LastAncientOne/Deep_Learning_Machine_Learning_Stock)

深度学习与机器学习在股票市场预测中的应用，涵盖特征工程与技术分析。

- **Stars:** ⭐️ 1.7k
- **Tags:** `stock-prediction` `deep-learning` `feature-engineering` `trading`
- **最后活动时间:** 2024-03-01

### [pmdarima](https://github.com/alkaline-ml/pmdarima)

Python时间序列分析库，提供R语言auto.arima的等效功能，支持ARIMA/SARIMAX模型自动拟合。

- **Stars:** ⭐️ 1.7k
- **Tags:** `time-series` `arima` `forecasting` `python`
- **最后活动时间:** 2025-11-17

### [AlphaPy](https://github.com/ScottfreeLLC/AlphaPy)

专为交易系统和体育博彩设计的Python AutoML框架。支持股票、加密货币的时间序列预测和回测。

- **Stars:** ⭐️ 1.7k
- **Tags:** `AutoML` `Trading` `Time Series`
- **最后活动时间:** 2025-08-24

### [RustQuant](https://github.com/avhz/RustQuant)

Rust语言编写的量化金融库，支持期权定价和统计分析。

- **Stars:** ⭐️ 1.7k
- **Tags:** `quantitative-finance` `option-pricing` `rust`
- **最后活动时间:** 2026-01-14

### [intelligent-trading-bot](https://github.com/asavinov/intelligent-trading-bot)

基于机器学习和特征工程的智能交易机器人，自动生成交易信号并执行交易策略。

- **Stars:** ⭐️ 1.6k
- **Tags:** `Machine-Learning` `Trading` `Crypto`
- **最后活动时间:** 2026-02-28

### [tigramite](https://github.com/jakobrunge/tigramite)

专注于时间序列数据的因果推断Python包，支持多种因果发现算法和统计分析方法。

- **Stars:** ⭐️ 1.6k
- **Tags:** `causal-inference` `time-series` `python` `statistics`
- **最后活动时间:** 2026-01-14

### [NBA-Machine-Learning-Sports-Betting](https://github.com/kyleskom/NBA-Machine-Learning-Sports-Betting)

使用机器学习和深度学习技术进行NBA体育赛事预测和投注分析。

- **Stars:** ⭐️ 1.6k
- **Tags:** `Machine Learning` `Sports Analytics` `Prediction`
- **最后活动时间:** 2026-01-09

### [openalgo](https://github.com/marketcalls/openalgo)

开源算法交易平台，支持多券商接入和策略自动化执行，适合量化交易开发者。

- **Stars:** ⭐️ 1.5k
- **Tags:** `algorithmic-trading` `quantitative-finance` `trading-bot` `python`
- **最后活动时间:** 2026-03-26

### [StockPricePrediction](https://github.com/scorpionhiccup/StockPricePrediction)

使用 LSTM、RNN 等机器学习技术进行股票价格预测的完整项目实现。

- **Stars:** ⭐️ 1.5k
- **Tags:** `stock-prediction` `lstm` `time-series` `machine-learning`
- **最后活动时间:** 2023-08-08

### [pytdx](https://github.com/rainx/pytdx)

通达信数据接口Python实现，用于获取A股实时行情与历史数据。

- **Stars:** ⭐️ 1.5k
- **Tags:** `stock-data` `trading` `finance` `python`
- **最后活动时间:** 2020-04-15

### [deep_trader](https://github.com/deependersingla/deep_trader)

使用强化学习进行股票交易的智能代理项目，探索AI在金融市场的应用。

- **Stars:** ⭐️ 1.5k
- **Tags:** `reinforcement-learning` `trading` `stock-market` `deep-learning`
- **最后活动时间:** 2018-01-22

### [Deep-Trading](https://github.com/Rachnog/Deep-Trading)

深度学习算法交易实验项目，探索神经网络在量化交易策略中的应用。

- **Stars:** ⭐️ 1.5k
- **Tags:** `algorithmic-trading` `deep-learning` `finance` `quantitative`
- **最后活动时间:** 2018-08-07

### [Personae](https://github.com/Ceruleanacg/Personae)

深度强化学习和监督学习在量化交易中的应用实现，包含多种交易策略和环境。

- **Stars:** ⭐️ 1.4k
- **Tags:** `reinforcement-learning` `quantitative-trading` `stock-prediction`
- **最后活动时间:** 2018-11-29

### [QuantAgent](https://github.com/Y-Research-SBU/QuantAgent)

面向量化交易的智能体框架，结合大语言模型进行金融分析。

- **Stars:** ⭐️ 1.4k
- **Tags:** `quantitative-trading` `agentic-ai` `finance`
- **最后活动时间:** 2026-02-03

### [Quantitative-Notebooks](https://github.com/LongOnly/Quantitative-Notebooks)

量化金融与算法交易教育笔记本合集，涵盖资产配置、交易策略、机器学习在金融中的应用。

- **Stars:** ⭐️ 1.3k
- **Tags:** `quantitative-finance` `algorithmic-trading` `jupyter` `machine-learning`
- **最后活动时间:** 2020-07-02

### [Introduction-to-Quantitative-Finance](https://github.com/Barca0412/Introduction-to-Quantitative-Finance)

量化金融入门资料合集，涵盖多因子框架教程、经典资料收录及AI+金融相关工作。

- **Stars:** ⭐️ 1.3k
- **Tags:** `Quantitative Finance` `LLM4Fin` `Trading`
- **最后活动时间:** 2026-03-25

### [jqdatasdk](https://github.com/JoinQuant/jqdatasdk)

简单易用的中国量化金融数据包，提供股票市场数据获取接口，支持Alpha因子计算，适用于量化交易和金融分析场景。

- **Stars:** ⭐️ 1.3k
- **Tags:** `financial-data` `quantitative-trading` `python` `stock-market`
- **最后活动时间:** 2026-01-29

### [stock-analysis-engine](https://github.com/AlgoTraders/stock-analysis-engine)

可扩展的算法交易回测引擎，支持深度学习训练和Kubernetes部署。

- **Stars:** ⭐️ 1.2k
- **Tags:** `algorithmic-trading` `backtesting` `deep-learning` `tensorflow` `kubernetes`
- **最后活动时间:** 2020-09-05

### [Stock.Indicators](https://github.com/DaveSkender/Stock.Indicators)

.NET股票技术指标库，提供算法交易、技术分析和机器学习所需的金融数据分析工具。

- **Stars:** ⭐️ 1.2k
- **Tags:** `stock-analysis` `technical-indicators` `algorithmic-trading` `quantitative-finance`
- **最后活动时间:** 2026-03-06

### [FinRL-Tutorials](https://github.com/AI4Finance-Foundation/FinRL-Tutorials)

FinRL强化学习金融交易教程，涵盖量化交易策略开发与AI驱动的金融市场分析。

- **Stars:** ⭐️ 1.2k
- **Tags:** `reinforcement-learning` `quantitative-trading` `finance` `deep-learning`
- **最后活动时间:** 2025-03-28

### [trading-bot](https://github.com/pskrunner14/trading-bot)

基于深度Q学习的股票交易机器人，实现智能量化交易策略。

- **Stars:** ⭐️ 1.1k
- **Tags:** `deep-q-learning` `stock-trading` `reinforcement-learning`
- **最后活动时间:** 2023-12-03

### [open-trading-api](https://github.com/koreainvestment/open-trading-api)

韩国投资证券开放API，集成LLM和MCP支持。提供实时交易、WebSocket接口，支持AI驱动的量化交易策略。

- **Stars:** ⭐️ 1.1k
- **Tags:** `量化交易` `API` `韩国证券`
- **最后活动时间:** 2026-03-18

### [AutoHedge](https://github.com/The-Swarm-Corporation/AutoHedge)

利用群体智能和AI代理构建自主对冲基金，自动化完成市场分析、风险管理和交易执行。

- **Stars:** ⭐️ 1.1k
- **Tags:** `ai` `hedgefund` `quant` `trading` `ai-agents`
- **最后活动时间:** 2026-03-09

### [deepdow](https://github.com/jankrepl/deepdow)

深度学习投资组合优化库，结合Markowitz理论与神经网络，支持时间序列预测。

- **Stars:** ⭐️ 1.1k
- **Tags:** `portfolio-optimization` `deep-learning` `finance` `trading`
- **最后活动时间:** 2024-01-24

### [listen](https://github.com/piotrostr/listen)

DeFAI 多功能工具箱，结合 AI 技术为 Solana 链上的 DEX 交易和 Meme 币提供智能化分析与操作支持。

- **Stars:** ⭐️ 1.1k
- **Tags:** `ai` `solana` `trading` `dex` `rust`
- **最后活动时间:** 2025-11-05

### [QuantDinger](https://github.com/brokermr810/QuantDinger)

AI驱动的本地优先量化交易平台，支持研究、回测和实盘交易，集成LLM和多智能体技术。

- **Stars:** ⭐️ 1.1k
- **Tags:** `AI-Trading` `LLM` `Multi-Agent`
- **最后活动时间:** 2026-03-25

### [AlgorithmicTrading](https://github.com/JerBouma/AlgorithmicTrading)

包含三种套利策略的算法交易项目：双重上市、期权和统计套利。

- **Stars:** ⭐️ 1.1k
- **Tags:** `Algorithmic Trading` `Arbitrage` `Finance` `Python`
- **最后活动时间:** 2023-08-13

### [alphagen](https://github.com/RL-MLDM/alphagen)

通过强化学习生成公式化股票Alpha因子，用于量化交易策略研究。

- **Stars:** ⭐️ 1.0k
- **Tags:** `quantitative-trading` `reinforcement-learning` `alpha-factors` `symbolic-regression`
- **最后活动时间:** 2024-12-18

### [CryptocurrencyPrediction](https://github.com/khuangaf/CryptocurrencyPrediction)

基于深度学习的加密货币价格预测系统，结合CNN、LSTM、GRU等模型。

- **Stars:** ⭐️ 1.0k
- **Tags:** `cryptocurrency` `deep-learning` `price-prediction`
- **最后活动时间:** 2023-03-10

### [btgym](https://github.com/Kismuz/btgym)

可扩展的深度学习友好型回测库，支持强化学习交易策略。

- **Stars:** ⭐️ 1.0k
- **Tags:** `backtesting` `reinforcement-learning` `trading` `tensorflow` `openai-gym`
- **最后活动时间:** 2021-08-28

### [atlas-gic](https://github.com/chrisworsey55/atlas-gic)

使用Karpathy风格自研究的自我改进AI交易智能体。

- **Stars:** ⭐️ 1.0k
- **Tags:** `trading-agents` `ai-trading` `autonomous`
- **最后活动时间:** 2026-03-20

### [ChatGPT_Trading_Bot](https://github.com/llSourcell/ChatGPT_Trading_Bot)

使用ChatGPT构建交易机器人的教程项目，展示AI在量化交易中的应用。

- **Stars:** ⭐️ 976
- **Tags:** `chatgpt` `trading-bot` `quantitative-trading`
- **最后活动时间:** 2023-01-29

### [Options-Trading-Strategies-in-Python](https://github.com/PyPatel/Options-Trading-Strategies-in-Python)

使用机器学习算法和技术指标开发期权交易策略，结合量化方法进行金融分析和投资决策。

- **Stars:** ⭐️ 976
- **Tags:** `machine-learning` `options-trading` `quantitative-finance` `trading-strategies`
- **最后活动时间:** 2024-04-22

### [crypto-rl](https://github.com/sadighian/crypto-rl)

深度强化学习工具包，用于加密货币限价订单簿数据记录与DDQN智能体训练。

- **Stars:** ⭐️ 947
- **Tags:** `deep-reinforcement-learning` `trading` `cryptocurrency` `dqn`
- **最后活动时间:** 2022-01-12

### [awesome-algorithmic-trading](https://github.com/joelowj/awesome-algorithmic-trading)

算法交易框架、库和资源的精选列表，涵盖机器学习驱动的量化交易策略和因子模型。

- **Stars:** ⭐️ 944
- **Tags:** `algorithmic-trading` `machine-learning` `quantitative-finance` `trading-strategies`
- **最后活动时间:** 2019-05-21

### [quant-notes](https://github.com/dingran/quant-notes)

量化金融面试准备指南，涵盖数学、编程和机器学习核心知识点。

- **Stars:** ⭐️ 925
- **Tags:** `quantitative-finance` `interview-preparation` `machine-learning` `statistics`
- **最后活动时间:** 2019-03-23

### [TradingView-Machine-Learning-GUI](https://github.com/TreborNamor/TradingView-Machine-Learning-GUI)

终端优先的TradingView策略实验室，支持下载市场数据、回测Python策略，并使用Optuna进行超参数优化。

- **Stars:** ⭐️ 919
- **Tags:** `algorithmic-trading` `backtesting` `hyperparameter-optimization` `trading-strategy`
- **最后活动时间:** 2026-03-16

### [Quant-Finance-Resources](https://github.com/PyPatel/Quant-Finance-Resources)

量化金融与AI结合的学习资源合集，涵盖算法交易、机器学习和股票预测等领域的课程与文章。

- **Stars:** ⭐️ 911
- **Tags:** `quantitative-finance` `algorithmic-trading` `machine-learning` `stock-price-prediction`
- **最后活动时间:** 2021-12-17

### [q-trader](https://github.com/edwardhdlu/q-trader)

基于深度Q学习的股票交易机器人，使用强化学习进行自动化交易决策。

- **Stars:** ⭐️ 906
- **Tags:** `reinforcement-learning` `trading-bot` `deep-q-learning` `finance`
- **最后活动时间:** 2023-11-26

### [qf-lib](https://github.com/quarkfin/qf-lib)

模块化量化金融Python库，提供高级事件驱动回测引擎和高质量量化交易工具。

- **Stars:** ⭐️ 902
- **Tags:** `quantitative-finance` `backtesting` `trading-strategies`
- **最后活动时间:** 2026-03-12

### [stock-scanner](https://github.com/DR-lin-eng/stock-scanner)

开源A股量化分析工具，结合LLM模型进行高级市场分析与投资决策支持。

- **Stars:** ⭐️ 899
- **Tags:** `quantitative-trading` `stock-analysis` `llm` `a-share`
- **最后活动时间:** 2026-03-02

### [LiuAlgoTrader](https://github.com/amor71/LiuAlgoTrader)

基于机器学习的算法交易框架，支持多交易所接入和智能交易策略开发。

- **Stars:** ⭐️ 890
- **Tags:** `Algorithmic Trading` `Machine Learning` `Python`
- **最后活动时间:** 2023-08-13

### [tradecat](https://github.com/tukuaiai/tradecat)

AI驱动的量化交易数据系统，支持实时市场数据分析、技术指标计算和Wyckoff分析方法。

- **Stars:** ⭐️ 878
- **Tags:** `AI` `Quantitative Trading` `Technical Analysis` `Binance`
- **最后活动时间:** 2026-03-11

### [FinMem-LLM-StockTrading](https://github.com/pipiku915/FinMem-LLM-StockTrading)

基于分层记忆和角色设计的LLM交易智能体，提升金融交易决策性能。

- **Stars:** ⭐️ 866
- **Tags:** `LLM` `Trading Agent` `Finance` `Memory`
- **最后活动时间:** 2024-08-18

### [Hyper-Alpha-Arena](https://github.com/HammerGPT/Hyper-Alpha-Arena)

AI驱动的自动交易平台，支持Hyperliquid和币安合约的智能交易策略。

- **Stars:** ⭐️ 856
- **Tags:** `ai-trading` `trading-bot` `cryptocurrency`
- **最后活动时间:** 2026-03-24

### [Stock-Trading-Environment](https://github.com/notadamking/Stock-Trading-Environment)

基于OpenAI Gym的股票交易模拟环境，支持历史价格数据上的强化学习交易策略训练。

- **Stars:** ⭐️ 850
- **Tags:** `reinforcement-learning` `openai-gym` `stock-trading` `simulation`
- **最后活动时间:** 2021-04-09

### [stock_predict_with_LSTM](https://github.com/hichenway/stock_predict_with_LSTM)

使用LSTM进行股票价格预测，支持PyTorch、Keras和TensorFlow框架，量化交易入门项目。

- **Stars:** ⭐️ 844
- **Tags:** `stock-prediction` `lstm` `time-series` `finance`
- **最后活动时间:** 2023-03-24

### [StockPredictionRNN](https://github.com/dzitkowskik/StockPredictionRNN)

使用LSTM循环神经网络进行高频交易价格预测的量化交易项目。

- **Stars:** ⭐️ 808
- **Tags:** `lstm` `stock-prediction` `rnn` `trading` `time-series`
- **最后活动时间:** 2016-05-21

### [finance_ml](https://github.com/jjakimoto/finance_ml)

《金融机器学习进展》配套资源，探索ML在金融领域的先进应用。

- **Stars:** ⭐️ 800
- **Tags:** `finance` `machine-learning` `quantitative`
- **最后活动时间:** 2023-01-11

### [stock_market_reinforcement_learning](https://github.com/kh-kim/stock_market_reinforcement_learning)

基于OpenAI Gym的股票市场环境，实现深度Q学习和策略梯度算法交易。

- **Stars:** ⭐️ 797
- **Tags:** `reinforcement-learning` `stock-market` `deep-q-learning` `policy-gradient`
- **最后活动时间:** 2016-12-23

### [deep-trading-agent](https://github.com/samre12/deep-trading-agent)

基于深度强化学习的比特币交易智能体，利用深度Q网络和循环神经网络进行自动化交易决策。

- **Stars:** ⭐️ 793
- **Tags:** `reinforcement-learning` `trading-agent` `bitcoin` `deep-q-network`
- **最后活动时间:** 2018-06-07

### [MAHORAGA](https://github.com/ygwyg/MAHORAGA)

基于社交媒体情绪分析和AI的自主交易智能体，具备学习和适应能力。

- **Stars:** ⭐️ 788
- **Tags:** `trading-agent` `sentiment-analysis` `autonomous-trading`
- **最后活动时间:** 2026-02-17

### [spectre](https://github.com/Heerozh/spectre)

GPU加速的因子分析库和回测框架，专为量化交易设计，支持高性能策略回测与分析。

- **Stars:** ⭐️ 786
- **Tags:** `algorithmic-trading` `backtesting` `gpu` `quantitative-finance`
- **最后活动时间:** 2025-04-15

### [wtte-rnn](https://github.com/ragulpr/wtte-rnn)

基于RNN的流失预测和时间事件预测框架，使用Weibull分布建模。

- **Stars:** ⭐️ 781
- **Tags:** `churn-prediction` `time-to-event` `rnn` `weibull` `tensorflow`
- **最后活动时间:** 2020-08-07

### [artificial-intelligence-for-trading](https://github.com/udacity/artificial-intelligence-for-trading)

Udacity AI交易纳米学位课程内容，涵盖量化交易、投资组合优化和AI在金融领域的应用。

- **Stars:** ⭐️ 769
- **Tags:** `ai-trading` `quantitative-trading` `machine-learning` `finance`
- **最后活动时间:** 2023-04-09

### [Neural-Net-with-Financial-Time-Series-Data](https://github.com/BenjiKCF/Neural-Net-with-Financial-Time-Series-Data)

使用TensorFlow进行金融时间序列深度学习的入门示例，展示机器学习在量化分析中的应用。

- **Stars:** ⭐️ 761
- **Tags:** `finance` `time-series` `tensorflow` `deep-learning`
- **最后活动时间:** 2020-12-15

### [bitpredict](https://github.com/cbyn/bitpredict)

基于机器学习的高频比特币价格预测项目，适用于量化交易研究。

- **Stars:** ⭐️ 760
- **Tags:** `bitcoin` `price-prediction` `machine-learning` `trading`
- **最后活动时间:** 2018-03-29

### [nof1-tracker](https://github.com/terryso/nof1-tracker)

命令行工具，用于追踪 nof1.ai AI Agent 交易信号并自动执行 Binance 期货交易，实现 AI 驱动的量化交易自动化。

- **Stars:** ⭐️ 746
- **Tags:** `ai-trading` `binance` `futures` `trading-bot` `cli`
- **最后活动时间:** 2025-11-28

### [Deep-Reinforcement-Stock-Trading](https://github.com/Albert-Z-Guo/Deep-Reinforcement-Stock-Trading)

轻量级深度强化学习投资组合管理框架，提供高度模块化和可扩展的股票交易解决方案。

- **Stars:** ⭐️ 692
- **Tags:** `deep-reinforcement-learning` `stock-trading` `portfolio-management` `quantitative-finance`
- **最后活动时间:** 2024-11-06

### [pylivetrader](https://github.com/alpacahq/pylivetrader)

Python实时交易执行库，提供zipline接口支持算法交易策略的实盘运行。

- **Stars:** ⭐️ 681
- **Tags:** `algorithmic-trading` `quant` `trading` `zipline`
- **最后活动时间:** 2022-10-04

### [stock-prediction-deep-neural-learning](https://github.com/JordiCorbilla/stock-prediction-deep-neural-learning)

使用TensorFlow LSTM神经网络进行股票价格时间序列预测。

- **Stars:** ⭐️ 669
- **Tags:** `stock-prediction` `lstm` `time-series` `finance` `tensorflow`
- **最后活动时间:** 2025-12-28

### [stock-trading-ml](https://github.com/yacoubb/stock-trading-ml)

使用深度学习(LSTM)进行股票价格预测的量化交易机器人项目。

- **Stars:** ⭐️ 664
- **Tags:** `deep-learning` `lstm` `stock-trading` `time-series` `machine-learning`
- **最后活动时间:** 2022-05-18

### [trade-frame](https://github.com/rburkholder/trade-frame)

C++17量化交易框架，集成libtorch/LSTM/CUDA支持深度学习策略，支持股票、期货、期权等多品种自动化交易。

- **Stars:** ⭐️ 651
- **Tags:** `C++` `LibTorch` `LSTM` `Quantitative-Trading`
- **最后活动时间:** 2026-03-05

### [longbridge-terminal](https://github.com/longbridge/longbridge-terminal)

AI原生交易平台命令行工具，提供实时行情数据、投资组合管理和交易功能。

- **Stars:** ⭐️ 649
- **Tags:** `ai-native` `trading` `cli` `tui`
- **最后活动时间:** 2026-03-26

### [trading-server](https://github.com/s-brez/trading-server)

多资产、多策略的事件驱动型量化交易平台，支持低中频策略在多个交易场所同时运行，具备组合风险管理和策略资本分配功能。

- **Stars:** ⭐️ 646
- **Tags:** `algorithmic-trading` `quantitative-trading` `trading-bot` `crypto-trading` `backtesting`
- **最后活动时间:** 2023-07-06

### [Real-time-stock-market-prediction](https://github.com/victor369basu/Real-time-stock-market-prediction)

基于TensorFlow.js和Kafka的实时股票预测系统，完整的机器学习服务端架构实现。

- **Stars:** ⭐️ 646
- **Tags:** `stock-prediction` `tensorflow-js` `kafka` `machine-learning`
- **最后活动时间:** 2021-02-03

### [akquant](https://github.com/akfamily/akquant)

基于Rust和Python的高性能量化研究与交易框架，支持回测和金融数据分析。

- **Stars:** ⭐️ 638
- **Tags:** `quantitative-trading` `backtest` `rust` `finance`
- **最后活动时间:** 2026-03-25

### [nof1.ai](https://github.com/195440/nof1.ai)

开源自主AI交易代理系统，提供智能化的量化交易解决方案。

- **Stars:** ⭐️ 629
- **Tags:** `ai-trading` `autonomous-agent` `quantitative-finance`
- **最后活动时间:** 2026-03-23

### [ContestTrade](https://github.com/FinStep-AI/ContestTrade)

基于内部竞赛机制的多智能体交易系统。

- **Stars:** ⭐️ 627
- **Tags:** `multi-agent` `trading` `quantitative-finance`
- **最后活动时间:** 2025-12-22

### [Stock-Price-Prediction-LSTM](https://github.com/NourozR/Stock-Price-Prediction-LSTM)

使用LSTM循环神经网络预测苹果公司股票OHLC平均价格的深度学习项目。

- **Stars:** ⭐️ 619
- **Tags:** `lstm` `stock-prediction` `time-series` `deep-learning`
- **最后活动时间:** 2023-03-30

### [Machine-Learning-for-Asset-Managers](https://github.com/emoen/Machine-Learning-for-Asset-Managers)

《Machine Learning for Asset Managers》书中的代码实现，专注于资产管理的机器学习应用。

- **Stars:** ⭐️ 616
- **Tags:** `machine-learning` `finance` `quantitative-trading`
- **最后活动时间:** 2026-02-11

### [trading-momentum-transformer](https://github.com/kieranjwood/trading-momentum-transformer)

基于Transformer的动量交易策略模型，可解释的量化交易架构。

- **Stars:** ⭐️ 604
- **Tags:** `transformer` `trading` `quantitative-finance` `deep-learning`
- **最后活动时间:** 2026-03-19

### [alpaca-mcp-server](https://github.com/alpacahq/alpaca-mcp-server)

Alpaca官方MCP服务器，让用户通过LLM工具直接用自然语言进行股票、ETF、加密货币和期权交易，支持数据分析和策略构建。

- **Stars:** ⭐️ 579
- **Tags:** `MCP` `AI Trading` `Quantitative Finance`
- **最后活动时间:** 2026-03-23

### [ML-HFT](https://github.com/bradleyboyuyang/ML-HFT)

期货高频交易框架，运用机器学习和深度学习技术处理订单簿数据，支持tick级别分析。

- **Stars:** ⭐️ 568
- **Tags:** `high-frequency-trading` `machine-learning` `futures` `orderbook`
- **最后活动时间:** 2022-09-20

### [Advanced-Deep-Trading](https://github.com/Rachnog/Advanced-Deep-Trading)

基于《Advances in Financial Machine Learning》书籍的金融机器学习实验项目，探索深度学习在交易中的应用。

- **Stars:** ⭐️ 566
- **Tags:** `financial-machine-learning` `deep-learning` `trading` `quantitative-finance`
- **最后活动时间:** 2020-11-29

### [rl-portfolio-management](https://github.com/wassname/rl-portfolio-management)

深度强化学习金融投资组合管理框架，复现学术论文并提供OpenAI Gym环境。

- **Stars:** ⭐️ 561
- **Tags:** `reinforcement-learning` `portfolio-management` `cryptocurrency` `openai-gym`
- **最后活动时间:** 2025-03-18

### [freqtrade-bot](https://github.com/Kaleighc793/freqtrade-bot)

开源加密货币量化交易机器人配置管理工具，支持策略回测、FreqAI机器学习策略优化、多交易所接入及Web界面管理。

- **Stars:** ⭐️ 559
- **Tags:** `Quantitative Trading` `Machine Learning` `Crypto Bot`
- **最后活动时间:** 2026-03-13

### [DeepLOB](https://github.com/zcakhaa/DeepLOB-Deep-Convolutional-Neural-Networks-for-Limit-Order-Books)

基于深度卷积神经网络的限价订单簿预测模型，用于金融市场高频交易分析。

- **Stars:** ⭐️ 553
- **Tags:** `limit-order-book` `cnn` `financial-trading` `deep-learning`
- **最后活动时间:** 2021-07-15

### [Stockagent](https://github.com/MingyuJ666/Stockagent)

基于大语言模型的股票交易智能体，在模拟真实世界环境中进行交易决策研究。

- **Stars:** ⭐️ 552
- **Tags:** `llm` `stock-trading` `reinforcement-learning` `simulation`
- **最后活动时间:** 2025-11-02

### [QuantaAlpha](https://github.com/QuantaAlpha/QuantaAlpha)

结合LLM智能与进化策略的量化因子挖掘框架，支持自动挖掘、演化和验证alpha因子。

- **Stars:** ⭐️ 548
- **Tags:** `量化交易` `因子挖掘` `LLM`
- **最后活动时间:** 2026-03-20

### [ai-hedge-fund-crypto](https://github.com/51bitquant/ai-hedge-fund-crypto)

基于LLM智能体的加密货币对冲基金系统，利用多智能体协作进行智能交易决策和投资组合管理。

- **Stars:** ⭐️ 544
- **Tags:** `AI Trading` `LLM Agent` `Crypto` `Hedge Fund`
- **最后活动时间:** 2025-09-05

### [Microservices-Based-Algorithmic-Trading-System](https://github.com/saeed349/Microservices-Based-Algorithmic-Trading-System)

基于Docker的量化交易平台，支持机器学习算法策略的开发、测试与部署。

- **Stars:** ⭐️ 536
- **Tags:** `algorithmic-trading` `machine-learning` `docker` `microservices`
- **最后活动时间:** 2024-04-08

### [deep-learning-bitcoin](https://github.com/philipperemy/deep-learning-bitcoin)

利用深度学习技术挖掘比特币价格模式，探索加密货币预测应用。

- **Stars:** ⭐️ 534
- **Tags:** `bitcoin` `price-prediction` `deep-learning` `cryptocurrency`
- **最后活动时间:** 2018-09-22

### [Stock-market-forecasting](https://github.com/pushpendughosh/Stock-market-forecasting)

使用LSTM和随机森林进行股票价格方向预测的日内交易模型。

- **Stars:** ⭐️ 532
- **Tags:** `stock-prediction` `lstm` `random-forest` `finance`
- **最后活动时间:** 2021-07-12

### [ai-trader](https://github.com/whchien/ai-trader)

基于Backtrader的算法交易回测框架，内置20+策略和MCP服务器支持。

- **Stars:** ⭐️ 528
- **Tags:** `algorithmic-trading` `backtrader` `mcp-server` `quantitative-finance`
- **最后活动时间:** 2026-02-04

### [finance-skills](https://github.com/himself65/finance-skills)

专为金融分析和交易设计的AI代理技能集合，助力构建智能金融助手。

- **Stars:** ⭐️ 526
- **Tags:** `finance` `ai-agents` `trading` `financial-analysis`
- **最后活动时间:** 2026-03-24

### [northstar](https://github.com/dromara/northstar)

基于Java的AI量化交易平台，支持历史回放、策略研发、模拟交易和实盘交易，可替代文华、MC等商业软件。

- **Stars:** ⭐️ 526
- **Tags:** `quantitative-trading` `ai` `futures` `strategy` `java`
- **最后活动时间:** 2026-03-21

### [QLearning_Trading](https://github.com/ucaiado/QLearning_Trading)

基于Q-Learning强化学习框架的量化交易系统，探索智能体在金融市场中的决策学习。

- **Stars:** ⭐️ 517
- **Tags:** `reinforcement-learning` `algorithmic-trading` `quantitative-finance`
- **最后活动时间:** 2016-10-15

### [mlbot_tutorial](https://github.com/richmanbtc/mlbot_tutorial)

使用机器学习构建算法交易机器人的教程，涵盖模型训练与交易策略实现。

- **Stars:** ⭐️ 514
- **Tags:** `machine-learning` `algorithmic-trading` `trading-bot` `finance`
- **最后活动时间:** 2022-11-28

### [gym-mtsim](https://github.com/AminHP/gym-mtsim)

OpenAI Gym兼容的MetaTrader 5交易模拟环境，专为强化学习算法设计，支持股票、加密货币和外汇交易策略训练。

- **Stars:** ⭐️ 512
- **Tags:** `Reinforcement Learning` `Trading` `OpenAI Gym`
- **最后活动时间:** 2024-11-14

### [prism-insight](https://github.com/dragon1086/prism-insight)

基于AI和LLM的股票分析与交易系统，采用多智能体架构和MCP协议。

- **Stars:** ⭐️ 510
- **Tags:** `Stock` `Multi-Agent` `MCP`
- **最后活动时间:** 2026-03-24

### [Quantitative-Research-Projects](https://github.com/Weizhi-Zhao-quant/Quantitative-Research-Projects)

量化金融研究项目合集，涵盖行业轮动、多因子模型及AI驱动的交易策略。

- **Stars:** ⭐️ 505
- **Tags:** `quantitative-finance` `machine-learning` `trading-strategies` `deep-learning`
- **最后活动时间:** 2026-01-16

